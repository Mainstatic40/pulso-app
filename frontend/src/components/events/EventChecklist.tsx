...
export function EventChecklist({ eventId, items, readOnly = false }: EventChecklistProps) {
  ...
  const addItemMutation = useMutation({
    mutationFn: (content: string) => eventService.addChecklistItem(eventId, content),
    onSuccess: () => {
  ...
  const updateItemMutation = useMutation({
    mutationFn: ({ itemId, data }: { itemId: string; data: { content?: string; isCompleted?: boolean } }) =>
      eventService.updateChecklistItem(eventId, itemId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['event', eventId] });
      setEditingId(null);
    },
  ...
  const deleteItemMutation = useMutation({
    mutationFn: (itemId: string) => eventService.deleteChecklistItem(eventId, itemId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['event', eventId] });
    },
  });
  ...
  const handleToggleComplete = (item: EventChecklistItem) => {
  ...
  const handleStartEdit = (item: EventChecklistItem) => {
  ...
  const handleDrop = (e: React.DragEvent, targetId: string) => {
    ...
    // For now, just reset drag state (reorder not implemented for events yet)
    setDraggedId(null);
    setDragOverId(null);
  };
