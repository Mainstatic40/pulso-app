        ...
        <div className="grid grid-cols-[60px_repeat(7,1fr)]">
          {HOURS.map((hour) => (
            <div key={hour} className="contents">
              <div className="sticky left-0 border-b border-r border-gray-200 bg-white py-2 text-right pr-2 text-xs text-gray-500">
                {formatHour(hour)}
              </div>
              {weekDays.map((day, dayIndex) => {
                ...
                return (
                  <div
                    key={`${hour}-${dayIndex}`}
                    className="min-h-[50px] border-b border-r border-gray-200 p-1 last:border-r-0"
                  >
